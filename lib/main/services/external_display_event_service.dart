...
class ExternalDisplayEventService {
  /* Static variables */

  static const String _channelName = 'com.anchwatt/external_display_events';
  // Native side already filters to actual screen-count changes, but we briefly
  // silence events immediately after start in case a stale notification leaks
  // through during the listener's initial wiring.
  ...
  Future<void> start() async {
    ...
    _nativeSubscription = _channel.receiveBroadcastStream().listen(
      _onNativeEvent,
      onError: (Object error) => debugPrint('ExternalDisplayEventService error: $error'),
    );
  }
